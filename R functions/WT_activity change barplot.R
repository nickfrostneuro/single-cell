...
#distrib.KO is the distribution of shuffled soc/not values for KO calculated by the "distribution_IEG.col2" function
#distrib.WT is the distribution of shuffled soc/not values for WT calculated by the "distribution_IEG.col2" function
#object is data frame with cell types as rows (in same order as CTs in distribution) and 3 columns (mean, sd, and sem of the distribution)

WT_barplot <- function(mIEG, md, Sub, distrib.KO, distrib.WT, object){
  ...
  #distrib <- distrib.WT/distrib.KO
  rm(d)
  distrib <- list()
  for(d in 1:ncol(distrib.WT)){
    distrib.KO <- distrib.KO[,c(2:ncol(distrib.KO),1)]
    dist <- distrib.WT/distrib.KO
    distrib[[d]] <- dist
  }
  distrib <- do.call(cbind, distrib)
  rownames(distrib) <- rownames(distrib.WT)
  
  #Calculate real values of WT col2/KO col2
  mat <- matrix(nrow=nrow(mIEG1), ncol = 1)
  for(i in 1:nrow(mIEG1)){
    if((i %% 2) == 0)next
    l <- as.numeric(mIEG1$val[(i)])/as.numeric(mIEG1$val[i+1])
    mat[i,] = l
  }
  WT.KO <- cbind(as.character(mIEG1$celltype), as.numeric(mat), rep(rev(object[,2]), each =2))
  row_odd <- seq_len(nrow(WT.KO)) %% 2 
  WT.KO <- WT.KO[row_odd == 1, ]
  colnames(WT.KO) <- c("celltype", "val", "sd")
  WT.KO <- as.data.frame(WT.KO)
  
  WT.KO <- WT.KO[nrow(WT.KO):1,]
  
  #Plot distributions by cell type with red line to show the real proportion increase value
  pval <- matrix(ncol = 1, nrow = nrow(distrib))
  for(i in 1:nrow(distrib)){
    #tfmin <- density(na.omit(as.numeric(distrib[i,])))
    #plot(tfmin, xlim = c(0,max(as.numeric(WT.KO$val)[is.finite(as.numeric(WT.KO$val))])), main = paste0(rownames(distrib.WT)[i], " KO/WT"))
    #abline(v = as.numeric(WT.KO$val[i]), col = "red")
    pval.tmp <- length(distrib[i,][(distrib[i,] > as.numeric(WT.KO$val[i]))])/length(distrib[i,])
    pval.tmp[pval.tmp == 0] =0.001
    pval[i,] = pval.tmp
  }
  rownames(pval) <- rownames(levels(Sub))
  WT.KO <- cbind(WT.KO, pval)
  WT.KO$condition <- rep("WT/KO", nrow(WT.KO))
  WT.KO <- WT.KO[nrow(WT.KO):1,]
  
  mIEG1.save <- mIEG1
  WT.KO.save <- WT.KO
  WT.KO$celltype <- factor(WT.KO$celltype, levels = rev(levels(Sub)))
  
  new_list <- list(mIEG1.save, WT.KO.save)
  names(new_list) <- c("mIEG1", "WT.KO")
  return(new_list)
}